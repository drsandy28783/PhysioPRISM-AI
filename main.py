...
# cred_dict['project_id'] = 'YOUR-PROJECT-ID'  # if missing
...
def provisional_diagnosis_suggest(patient_id):
    ...
    try:
        prompt = build_provisional_diagnosis_prompt(patient_id, field, patient)
        suggestion = get_ai_suggestion(prompt).strip()
...
def ai_followup_suggestion(patient_id):
    from prompts_py import generate_followup_prompt
